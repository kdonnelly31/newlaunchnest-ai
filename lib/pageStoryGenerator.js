...
// section type repeats, and every imageId a section references must be one
// that was ACTUALLY shown to the model -- checked against `manifest` (built
// server-side from the real Etsy listing data), not against the AI's own
// self-reported `images[]`, since an AI that hallucinates a self-consistent
// id/images[] pair would otherwise pass validation despite referencing
// nothing real. Also merges the manifest's real URLs onto each classified
// image before returning -- an image src/href on the rendered page must
// never be a value the AI authored, only a URL the server already trusted
// enough to show the AI in the first place.
export function validatePagePlan(pagePlan, manifest) {
  ...
  const referencedImageIds = sections.flatMap(section => {
    ...
    if ('imageId' in section && section.imageId != null) return [section.imageId];
    return [];
  });
