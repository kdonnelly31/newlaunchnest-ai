...
test('a second sync run is skipped while a lock from the first run is still fresh', async () => {
  const store = createFakeStore();
  // Seeding a held lock: the store starts unlocked, so any staleBefore works.
  await store.acquireLock({ nowIso: new Date().toISOString(), staleBeforeIso: new Date(0).toISOString() });
...
test('a stale lock (older than 5 minutes) does not block a new run', async () => {
  ...
  await store.acquireLock({ nowIso: staleTime, staleBeforeIso: new Date(0).toISOString() });
...
// Documents the acquire contract the real store must uphold. The in-memory
// fake is single-threaded, so this can't prove Postgres-level atomicity --
// that comes from the single conditional UPDATE in lib/mtoStore.js, which
// needs a live database to exercise, like the rest of that file.
test('acquireLock is atomic: only one of two concurrent callers succeeds', async () => {
  const store = createFakeStore();
  const nowIso = new Date().toISOString();
  const staleBeforeIso = new Date(0).toISOString();
  const [a, b] = await Promise.all([
    store.acquireLock({ nowIso, staleBeforeIso }),
    store.acquireLock({ nowIso, staleBeforeIso }),
  ]);
